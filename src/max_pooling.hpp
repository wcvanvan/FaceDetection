...
bool max_pooling(Matrix<float>& matrix_in, Matrix<float>& result_matrix) {
	...
	size_t output_area = output_w * output_h;
	...
	for (size_t output_channel = 0; output_channel < matrix_in.channels; output_channel++)
	{
		for (int output_row = 0; output_row < output_h; ++output_row) {
			...
			for (int output_col = 0; output_col < output_w; ++output_col) {
				...
				float numbers[3];
				numbers[0] = matrix_in.data_start[output_channel * output_area + input_row * matrix_in.colsROI + input_col + 1];
				numbers[1] = matrix_in.data_start[output_channel * output_area + (input_row + 1) * matrix_in.colsROI + input_col];
				numbers[2] = matrix_in.data_start[output_channel * output_area + (input_row + 1) * matrix_in.colsROI + input_col + 1];
				float max = matrix_in.data_start[output_channel * output_area + input_row * matrix_in.colsROI + input_col];
